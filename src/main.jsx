...
function DatasetImporter({ token, onComplete }) {
  const examples = {
    tourismStatistics: '[\n  {"destinationId":"munnar", "date":"2026-01-01", "visitors":4200, "geographicLevel":"destination", "modelEligible":false}\n]',
    regionalTourismStatistics: '[\n  {"region":"Kerala", "date":"2024-12-31", "domesticVisitsMillions":22.247, "foreignVisitsMillions":0.738}\n]',
    behaviour: '[\n  {"userId":"participant-001", "destinationId":"hampi", "date":"2026-01-01", "action":"rated", "rating":4, "researchConsent":true, "deidentified":true, "consentReference":"approved-study-protocol"}\n]'
  };
  const [dataset, setDataset] = useState('tourismStatistics'); const [sourceName, setSourceName] = useState('Official dataset import'); const [sourceUrl, setSourceUrl] = useState(''); const [records, setRecords] = useState(examples.tourismStatistics); const [message, setMessage] = useState('');
  const chooseDataset = value => { setDataset(value); setRecords(examples[value] || '[\n  {}\n]'); setMessage(''); };
  const submit = async event => {
    event.preventDefault();
  ...
  return <section className="panel import-panel"><p className="eyebrow">VALIDATED DATA IMPORT</p><h2>Replace prototype data responsibly</h2><form onSubmit={submit}><div className="import-meta"><label>Dataset<select value={dataset} onChange={event => chooseDataset(event.target.value)}><option value="tourismStatistics">Destination tourism observations</option><option value="regionalTourismStatistics">State/UT tourism context</option><option value="environmentReadings">Historical environment readings</option><option value="heritageSites">UNESCO / heritage sites</option><option value="festivals">Festivals</option><option value="destinations">Destinations</option><option value="behaviour">Tourist behaviour</option></select></label><label>Source name<input value={sourceName} onChange={event => setSourceName(event.target.value)} required /></label><label>Source URL<input value={sourceUrl} onChange={event => setSourceUrl(event.target.value)} placeholder="https://…" /></label></div><label>JSON records<textarea value={records} onChange={event => setRecords(event.target.value)} spellCheck="false" /></label><button>Validate & import</button></form>{message && <p className="notice">{message}</p>}<p className="hint">State/UT totals are stored as context and never train the destination model. Behaviour records count as research evidence only when they are de-identified and include an approved consent reference.</p></section>;
}

function ResearchReadiness({ readiness }) {
  if (!readiness) return <section className="panel"><p className="eyebrow">RESEARCH READINESS</p><h2>Checking evidence…</h2></section>;
  return <section className="panel"><p className="eyebrow">RESEARCH READINESS</p><h2>{readiness.overallReady ? 'Evidence ready for reporting' : `${readiness.blockers.length} evidence task${readiness.blockers.length === 1 ? '' : 's'} remain`}</h2><div className="readiness-list">{readiness.checks.map(check => <p className="source" key={check.id}><span className={check.ready ? 'on' : 'off'} />{check.label}<b>{check.detail}</b></p>)}</div><p className="hint">Only approved destination-level data can replace the synthetic crowd-model history. Browser interactions are intentionally excluded from research evaluation unless imported with consent and de-identification metadata.</p></section>;
}
