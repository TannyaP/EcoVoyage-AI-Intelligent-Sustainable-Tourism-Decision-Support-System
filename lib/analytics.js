...
function trainCrowdModel(destinations) {
  const rows = generateSyntheticHistory(destinations);
  const shuffled = [...rows];
  ...
  return {
    ...
    dataLabel: 'Synthetic historical visitor data - replace with India tourism statistics before research publication'
  };
}
...
function capacityAssessment(destination, predictedCrowd) {
  ...
  const safeCapacity = Math.round(destination.capacity * weatherPenalty * airPenalty * heritagePenalty);
  ...
  return {
    ...
    factors: { weatherPenalty: round(weatherPenalty, 2), airPenalty: round(airPenalty, 2), heritageProtection: round(heritagePenalty, 2) }
  };
}
